...
struct exception_ptr_rethrow_null_fn {
...
template <typename T>
T* exception_ptr_addressof(T& ref) noexcept {
...
inline std::type_info const* exception_ptr_get_type(
  ...
  if (!exception_ptr_access()) {
    auto const try_ = detail::exception_ptr_rethrow_null_fn{ptr};
    auto const catch_ = detail::exception_ptr_exception_typeid;
    return catch_exception<std::exception const&>(try_, catch_);
  }
...
T* exception_ptr_get_object(std::exception_ptr const& ptr) noexcept {
  ...
  if (!exception_ptr_access()) {
    auto const try_ = detail::exception_ptr_rethrow_null_fn{ptr};
    auto const catch_ = detail::exception_ptr_addressof<T>;
