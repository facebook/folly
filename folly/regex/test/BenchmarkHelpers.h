...
struct PcreCtx {
  ...
  bool search(std::string_view input) const {
    int ov[30];
    return pcre_exec(re, extra, input.data(), input.size(), 0, 0, ov, 30) >= 0;
  }

  bool match(std::string_view input) const {
    int ov[30];
    int rc = pcre_exec(
        re, extra, input.data(), input.size(), 0, PCRE_ANCHORED, ov, 30);
...
static constexpr std::size_t kRecursiveEngineMaxInput = 8192;
