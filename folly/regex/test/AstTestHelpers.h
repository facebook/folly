...
// Count Repeat nodes with RepeatMode::PossessiveProbed in the subtree.
constexpr int countPossessiveProbed(const auto& ast, int nodeIdx) noexcept {
  if (nodeIdx < 0) {
    return 0;
  }
  const auto& node = ast.nodes[nodeIdx];
  int count =
      (node.kind == NodeKind::Repeat && node.isPossessiveProbed()) ? 1 : 0;
  switch (node.kind) {
    case NodeKind::Sequence:
    case NodeKind::Alternation: {
      int child = node.child_first;
      while (child >= 0) {
        count += countPossessiveProbed(ast, child);
        child = ast.nodes[child].next_sibling;
      }
      break;
    }
    ...
    case NodeKind::Lookbehind:
    case NodeKind::NegLookbehind:
      count += countPossessiveProbed(ast, node.child_first);
      break;
    case NodeKind::Empty:
    case NodeKind::Literal:
    case NodeKind::AnyByte:
...
constexpr int countPossessiveProbed(const auto& ast) noexcept {
  return countPossessiveProbed(ast, ast.root);
}

// Count Repeat nodes with exact RepeatMode::Possessive (not Probed).
constexpr int countPossessiveExact(const auto& ast, int nodeIdx) noexcept {
  if (nodeIdx < 0) {
    return 0;
  }
  const auto& node = ast.nodes[nodeIdx];
  int count =
      (node.kind == NodeKind::Repeat &&
       node.repeat_mode == detail::RepeatMode::Possessive)
      ? 1
      : 0;
  switch (node.kind) {
    case NodeKind::Sequence:
    case NodeKind::Alternation: {
      int child = node.child_first;
      while (child >= 0) {
        count += countPossessiveExact(ast, child);
        child = ast.nodes[child].next_sibling;
      }
      break;
    }
    ...
    case NodeKind::Lookbehind:
    case NodeKind::NegLookbehind:
      count += countPossessiveExact(ast, node.child_first);
      break;
    case NodeKind::Empty:
    case NodeKind::Literal:
    case NodeKind::AnyByte:
...
constexpr int countPossessiveExact(const auto& ast) noexcept {
  return countPossessiveExact(ast, ast.root);
}
