...
// ===== Targeted benchmarks for new optimizations =====

FOLLY_NOINLINE const std::string& getPossessiveInput() {
  static const std::string s = "abcdef123x";
  return s;
}
FOLLY_NOINLINE const std::string& getWordSpaceInput() {
  static const std::string s = "hello world";
  return s;
}
FOLLY_NOINLINE const std::string& getBranchSubsumeInput() {
  static const std::string s = "abc123z";
  return s;
}
...
MATCH_BENCH(PossessiveInfer_Disjoint, "[a-z]+[0-9]+x", getPossessiveInput());
MATCH_BENCH(PossessiveInfer_WordSpace, "\\w+\\s+\\w+", getWordSpaceInput());
MATCH_BENCH(
    BranchSubsume_WordDigit, "(?:\\w+|\\d+)+z", getBranchSubsumeInput());
