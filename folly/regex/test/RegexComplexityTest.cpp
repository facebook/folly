...
//
// Optimization runs on the mutable AstBuilder before producing the
// fixed-size ParseResult, mirroring the compile<>() pipeline in Regex.h.
...
// Accesses holders directly to inspect compilation artifacts regardless
// of whether Regex skips automata for backtrack-safe patterns.
template <FixedPattern Pat>
struct FullRegexInfo {
  using R = Regex<Pat>;
  static constexpr Flags kCompFlags = compilationFlags(Flags::None);
  using NfaH = detail::NfaHolder<Pat, kCompFlags>;
  using DfaH = detail::DfaHolder<Pat, kCompFlags>;
  static constexpr int nfaStates = NfaH::nfaProg_.state_count;
  static constexpr bool dfaValid = DfaH::dfaProg_.valid;
  static constexpr int maxDfaStates = DfaH::kMaxDfaStates;
};
