...
constexpr int extractBranchLiteralSuffix(
  ...
  switch (node.kind) {
    ...
    case NodeKind::Sequence: {
      // Walk to last child, then traverse backward via prev_sibling
      int last = getLastSibling(ast, node.child_first);
      int total = 0;
  ...
  switch (node.kind) {
    ...
    case NodeKind::Sequence: {
      ...
      for (int i = 0; i < childCount && remaining > 0; ++i) {
        ...
        } else if (cnode.kind == NodeKind::Group) {
          char tmp[256] = {};
          int extractable =
      ...
      if (newChildCount == 1) {
        ast.nodes[newChildren[0]].next_sibling = -1;
        return newChildren[0];
      }

      linkSiblingList(ast, newChildren, newChildCount);

      AstNode seq;
      seq.kind = NodeKind::Sequence;
      seq.child_first = newChildren[0];
      seq.child_last = newChildren[newChildCount - 1];
    ...
    case NodeKind::Group: {
      ...
      grp.capturing = node.capturing;
      grp.group_id = node.group_id;
...
constexpr int stripLiteralSuffix(auto& ast, int nodeIdx, int count) noexcept {
  ...
  switch (node.kind) {
    ...
    case NodeKind::Sequence: {
      ...
      linkSiblingList(ast, children, keepCount);
      ...
      seq.child_last = children[keepCount - 1];
      return ast.addNode(seq);
    }
    case NodeKind::Group: {
      if (node.capturing) {
...
constexpr int factorAlternationLiteral(auto& ast, int altNodeIdx) noexcept {
  ...
  char extracted[kMaxAltBranches][kMaxExtractLen] = {};
  int extractedLens[kMaxAltBranches] = {};

  for (int i = 0; i < branchCount; ++i) {
    if constexpr (IsSuffix) {
      extractedLens[i] = extractBranchLiteralSuffix(
          ast, branches[i], extracted[i], kMaxExtractLen);
    } else {
      extractedLens[i] = extractBranchLiteralPrefix(
          ast, branches[i], extracted[i], kMaxExtractLen);
    }
  }
  ...
  for (int i = 0; i < branchCount; ++i) {
    ...
    if (extractedLens[i] > 0) {
      for (int j = i + 1; j < branchCount; ++j) {
        if (processed.test(j)) {
          continue;
        }
        if (extractedLens[j] > 0 && extracted[j][0] == extracted[i][0]) {
          group[groupCount++] = j;
        }
      }
    }
    ...
    int commonLen = extractedLens[group[0]];
    for (int g = 1; g < groupCount; ++g) {
      int bIdx = group[g];
      int len =
          extractedLens[bIdx] < commonLen ? extractedLens[bIdx] : commonLen;
      int match = 0;
      while (
          match < len && extracted[group[0]][match] == extracted[bIdx][match]) {
        ++match;
      }
      commonLen = match;
    }
    ...
    linkSiblingList(ast, strippedBranches, groupCount);

    AstNode innerAlt;
    innerAlt.kind = NodeKind::Alternation;
    innerAlt.child_first = strippedBranches[0];
    innerAlt.child_last = strippedBranches[groupCount - 1];
    innerAlt.next_sibling = -1;
    ...
    grp.kind = NodeKind::Group;
    grp.capturing = false;
    grp.child_first = innerAltIdx;
    grp.child_last = innerAltIdx;
    ...
    if constexpr (IsSuffix) {
      ...
      for (int c = 0; c < commonLen; ++c) {
        suffChars[c] = extracted[group[0]][commonLen - 1 - c];
      }
      AstNode lit;
      lit.kind = NodeKind::Literal;
      lit.literal = ast.appendLiteral(suffChars, commonLen);
      int litIdx = ast.addNode(lit);
      ast.nodes[litIdx].next_sibling = -1;

      // Sequence: group(alternation) -> suffix literal
      ast.nodes[grpIdx].next_sibling = litIdx;
      ast.nodes[litIdx].prev_sibling = grpIdx;

      AstNode seq;
      seq.kind = NodeKind::Sequence;
      seq.child_first = grpIdx;
      seq.child_last = litIdx;
      seq.next_sibling = -1;
      ...
      AstNode lit;
      lit.kind = NodeKind::Literal;
      lit.literal = ast.appendLiteral(extracted[group[0]], commonLen);
      int litIdx = ast.addNode(lit);

      // Sequence: prefix literal -> group(alternation)
      ast.nodes[litIdx].next_sibling = grpIdx;
      ast.nodes[grpIdx].prev_sibling = litIdx;
      ast.nodes[grpIdx].next_sibling = -1;

      AstNode seq;
      seq.kind = NodeKind::Sequence;
      seq.child_first = litIdx;
      seq.child_last = grpIdx;
      seq.next_sibling = -1;
      int seqIdx = ast.addNode(seq);
      newBranches[newBranchCount++] = seqIdx;
    }
  ...
  linkSiblingList(ast, newBranches, newBranchCount);

  AstNode newAlt;
  newAlt.kind = NodeKind::Alternation;
  newAlt.child_first = newBranches[0];
  newAlt.child_last = newBranches[newBranchCount - 1];
  newAlt.next_sibling = -1;
...
constexpr int recordPrefixGroupAdjustments(
    auto& ast, int nodeIdx, int maxLen, int currentOffset) noexcept {
  if (nodeIdx < 0 || maxLen <= 0) {
    return 0;
  }
...
// Only safe when !has_lookbehind.
// --------------------------------------------------------------------------

constexpr void extractRootLiteralPrefix(auto& ast) noexcept {
...
// with the following node. Mirrors computeSuffixStripLen but checks the
// node that follows the prefix instead of the node that precedes the suffix.
//
...
constexpr void computePrefixStripLen(auto& ast) noexcept {
  ...
  for (int c = followingNode; c >= 0; c = ast.nodes[c].next_sibling) {
    auto f = extractFollowFilter(ast, c);
    if (f.accepts_all) {
  ...
  if (followingFilter.range_count == 0) {
    ast.prefix_strip_len = ast.prefix_len;
    return;
  }
...
constexpr int extractSuffixChars(
  ...
  switch (node.kind) {
    ...
    case NodeKind::Sequence: {
      ...
      for (int c = last; c >= 0 && total < maxLen;
           c = ast.nodes[c].prev_sibling) {
        int got = extractSuffixChars(ast, c, out + total, maxLen - total);
        if (got == 0) {
          break;
        }
        total += got;
        if (ast.nodes[c].kind != NodeKind::Literal) {
          break;
        }
      }
      return total;
    ...
    case NodeKind::Group:
      return extractSuffixChars(ast, node.child_first, out, maxLen);
    case NodeKind::Repeat:
    case NodeKind::Anchor:
    case NodeKind::WordBoundary:
    case NodeKind::NegWordBoundary:
...
// --------------------------------------------------------------------------
// computeSuffixStripLen: determine how many trailing suffix characters can
// be safely stripped from the AST based on disjointness with preceding
// nodes. Walks right-to-left through the root sequence to build the
// "preceding character set", then scans suffix chars left-to-right for
// the first disjoint character (barrier). Everything from the barrier
// onward can be stripped.
// --------------------------------------------------------------------------

constexpr void computeSuffixStripLen(auto& ast) noexcept {
  if (ast.suffix_len <= 0 || ast.root < 0) {
    return;
  }
  ...
  if (ast.nodes[rootIdx].kind != NodeKind::Sequence) {
    ast.suffix_strip_len = ast.suffix_len;
    return;
  }

  // Walk to last child
  int lastChild = getLastSibling(ast, ast.nodes[rootIdx].child_first);
  if (lastChild < 0) {
    return;
  }

  int suffixRemaining = ast.suffix_len;
  NodeIdx precedingNode = kNoNode;

  for (int c = lastChild; c >= 0 && suffixRemaining > 0;
       c = ast.nodes[c].prev_sibling) {
    char tmp[256] = {};
    int cap = suffixRemaining < 256 ? suffixRemaining : 256;
    int extracted = extractSuffixChars(ast, c, tmp, cap);
    ...
    suffixRemaining -= extracted;
    if (suffixRemaining <= 0) {
      precedingNode = ast.nodes[c].prev_sibling;
      break;
    }
  }
  ...
  FirstCharFilter precedingFilter;
  precedingFilter.accepts_all = false;

  bool foundRequiredChild = false;
  for (int c = precedingNode; c >= 0; c = ast.nodes[c].prev_sibling) {
    auto f = extractFollowFilter(ast, c);
    if (f.accepts_all) {
      precedingFilter.accepts_all = true;
      break;
    }
    precedingFilter.mergeFrom(f.ranges, f.range_count);
    if (precedingFilter.accepts_all) {
      break;
    }
    if (!isPossiblyZeroWidth(ast, c)) {
      foundRequiredChild = true;
  ...
  // If all preceding children are possibly zero-width, the entire preceding
  // content can match empty. The suffix could be at the very start of the
  // match, preceded by any input character — stripping is unsafe.
  if (!foundRequiredChild && !precedingFilter.accepts_all) {
    precedingFilter.accepts_all = true;
  }

  if (precedingFilter.accepts_all) {
    ast.suffix_strip_len = 0;
    return;
  }

  if (precedingFilter.range_count == 0) {
    ast.suffix_strip_len = ast.suffix_len;
    return;
  }

  int suffStart = ast.literal_buf_size - ast.suffix_len;
  for (int i = 0; i < ast.suffix_len; ++i) {
    auto c = static_cast<unsigned char>(ast.literal_buf[suffStart + i]);
    bool overlaps = false;
    for (int r = 0; r < precedingFilter.range_count; ++r) {
      if (c >= precedingFilter.ranges[r].lo &&
          c <= precedingFilter.ranges[r].hi) {
        overlaps = true;
        break;
      }
    }
    if (!overlaps) {
      ast.suffix_strip_len = ast.suffix_len - i;
      return;
    }
  }
...
// --------------------------------------------------------------------------
// stripRootLiteralSuffix: strip suffix_strip_len trailing literal
// characters from the AST root using the existing stripLiteralSuffix.
// --------------------------------------------------------------------------

constexpr void stripRootLiteralSuffix(auto& ast) noexcept {
  if (ast.suffix_strip_len <= 0 || ast.root < 0) {
    return;
  }
  int oldRoot = ast.root;
  ast.root = stripLiteralSuffix(ast, ast.root, ast.suffix_strip_len);
  if (ast.root == oldRoot) {
    ast.suffix_strip_len = 0;
  }
}
...
constexpr int removeDuplicateBranches(auto& ast, int altNodeIdx) noexcept {
  ...
  if (keptCount == 1) {
    ast.nodes[kept[0]].next_sibling = -1;
    return kept[0];
  }

  linkSiblingList(ast, kept, keptCount);

  AstNode newAlt;
  newAlt.kind = NodeKind::Alternation;
  newAlt.child_first = kept[0];
  newAlt.child_last = kept[keptCount - 1];
  newAlt.next_sibling = -1;
  return ast.addNode(newAlt);
}
...
constexpr int eliminateSubsumedBranches(auto& ast, int altNodeIdx) noexcept {
  ...
  linkSiblingList(ast, kept, ki);

  AstNode newAlt;
  newAlt.kind = NodeKind::Alternation;
  newAlt.child_first = kept[0];
  newAlt.child_last = kept[ki - 1];
  newAlt.next_sibling = -1;
  return ast.addNode(newAlt);
}
...
constexpr int hoistCommonAnchor(auto& ast, int altNodeIdx) noexcept {
  ...
  for (int i = 0; i < branchCount; ++i) {
    ...
    if (bnode.kind == NodeKind::Anchor) {
      ...
      if (secondChild < 0) {
        ...
        if (thirdChild < 0) {
          ast.nodes[secondChild].next_sibling = -1;
  ...
  linkSiblingList(ast, branches, branchCount);

  AstNode innerAlt;
  innerAlt.kind = NodeKind::Alternation;
  innerAlt.child_first = branches[0];
  innerAlt.child_last = branches[branchCount - 1];
  innerAlt.next_sibling = -1;
  int innerAltIdx = ast.addNode(innerAlt);
  ...
  ast.nodes[anchorIdx].next_sibling = grpIdx;
  ast.nodes[grpIdx].prev_sibling = anchorIdx;
  ast.nodes[grpIdx].next_sibling = -1;

  AstNode seq;
  seq.kind = NodeKind::Sequence;
  seq.child_first = anchorIdx;
  seq.child_last = grpIdx;
  seq.next_sibling = -1;
  return ast.addNode(seq);
}
...
constexpr int stripNodePrefix(auto& ast, int nodeIdx, int count) noexcept {
  ...
  if (node.kind == NodeKind::Sequence) {
    ...
    if (remaining == 1) {
      ast.nodes[children[count]].next_sibling = -1;
      ast.nodes[children[count]].prev_sibling = kNoNode;
      return children[count];
    }
    linkSiblingList(ast, children + count, remaining);
    AstNode seq;
    seq.kind = NodeKind::Sequence;
    seq.child_first = children[count];
    seq.child_last = children[count + remaining - 1];
    return ast.addNode(seq);
  }
  if (node.kind == NodeKind::Group && !node.capturing) {
    int inner = stripNodePrefix(ast, node.child_first, count);
    if (inner == node.child_first) {
      return nodeIdx;
    }
    if (ast.nodes[inner].kind == NodeKind::Empty) {
...
constexpr int stripNodeSuffix(auto& ast, int nodeIdx, int count) noexcept {
  ...
  if (node.kind == NodeKind::Sequence) {
    ...
    if (keepCount == 1) {
      ast.nodes[children[0]].next_sibling = -1;
      return children[0];
    }
    linkSiblingList(ast, children, keepCount);
    AstNode seq;
  ...
  if (node.kind == NodeKind::Group && !node.capturing) {
    int inner = stripNodeSuffix(ast, node.child_first, count);
    if (inner == node.child_first) {
      return nodeIdx;
    }
    if (ast.nodes[inner].kind == NodeKind::Empty) {
...
template <bool IsSuffix>
constexpr int factorGeneralized(auto& ast, int altNodeIdx) noexcept {
  if (ast.nodes[altNodeIdx].kind != NodeKind::Alternation) {
    return altNodeIdx;
  }
  ...
  for (int i = 0; i < branchCount; ++i) {
    ...
    if constexpr (IsSuffix) {
      targetChild[i] = childCount > 0 ? children[childCount - 1] : -1;
    } else {
      targetChild[i] = childCount > 0 ? children[0] : -1;
    }
  }
  ...
  for (int i = 0; i < branchCount; ++i) {
    ...
    if constexpr (IsSuffix) {
      ...
      for (int p = 0; p < commonLen; ++p) {
        ...
        ast.nodes[prevIdx].next_sibling = copyIdx;
        ast.nodes[copyIdx].prev_sibling = prevIdx;
        prevIdx = copyIdx;
      }
      ast.nodes[prevIdx].next_sibling = -1;

      AstNode seq;
      seq.kind = NodeKind::Sequence;
      seq.child_first = grpIdx;
      seq.child_last = prevIdx;
      seq.next_sibling = -1;
      int seqIdx = ast.addNode(seq);
      newBranches[newBranchCount++] = seqIdx;
    } else {
      ...
      for (int p = 0; p < commonLen; ++p) {
        ...
        if (prevCopyIdx >= 0) {
          ast.nodes[prevCopyIdx].next_sibling = copyIdx;
          ast.nodes[copyIdx].prev_sibling = prevCopyIdx;
        }
        prevCopyIdx = copyIdx;
      }
      ast.nodes[prevCopyIdx].next_sibling = grpIdx;
      ast.nodes[grpIdx].prev_sibling = prevCopyIdx;
      ast.nodes[grpIdx].next_sibling = -1;

      AstNode seq;
      seq.kind = NodeKind::Sequence;
      seq.child_first = firstCopyIdx;
      seq.child_last = grpIdx;
      seq.next_sibling = -1;
      int seqIdx = ast.addNode(seq);
      newBranches[newBranchCount++] = seqIdx;
...
constexpr int optimizeNode(auto& ast, int nodeIdx) noexcept {
  ...
  switch (node.kind) {
    case NodeKind::Sequence: {
      ...
      int prevChild = -1;
      while (child >= 0) {
        int nextSib = ast.nodes[child].next_sibling;
        int optimized = optimizeNode(ast, child);
        if (optimized != child) {
          ast.nodes[optimized].next_sibling = nextSib;
          ast.nodes[optimized].prev_sibling =
              prevChild >= 0 ? prevChild : kNoNode;
          if (nextSib >= 0) {
            ast.nodes[nextSib].prev_sibling = optimized;
          }
          if (prevChild >= 0) {
            ast.nodes[prevChild].next_sibling = optimized;
          } else {
            node.child_first = optimized;
          }
          child = optimized;
        }
        prevChild = child;
      ...
      // Adjacent repeat merging: x+x+ → x{2,∞}
      {
        ...
        while (cur >= 0) {
          ...
          if (next >= 0 && ast.nodes[cur].kind == NodeKind::Repeat &&
            ...
            merged.next_sibling = ast.nodes[cur].child_first;
            int mergedIdx = ast.addNode(merged);
            ast.nodes[mergedIdx].next_sibling = ast.nodes[next].next_sibling;
            ast.nodes[mergedIdx].prev_sibling = prev2 >= 0 ? prev2 : kNoNode;
            if (ast.nodes[next].next_sibling >= 0) {
              ast.nodes[ast.nodes[next].next_sibling].prev_sibling = mergedIdx;
            }
            if (prev2 >= 0) {
              ast.nodes[prev2].next_sibling = mergedIdx;
            } else {
              node.child_first = mergedIdx;
            }
            cur = mergedIdx;
          } else {
            prev2 = cur;
    ...
    case NodeKind::Group: {
      int inner = optimizeNode(ast, node.child_first);
      if (inner != node.child_first) {
    ...
    case NodeKind::Repeat: {
      ...
      if (node.min_repeat == 1 && node.max_repeat == 1) {
        if (node.isPossessive()) {
          // Atomic group desugared as {1,1}+ — propagate possessive inward
          int child = node.child_first;
          ...
          if (ast.nodes[child].kind == NodeKind::Repeat) {
            ...
            ast.nodes[child].repeat_mode = node.repeat_mode;
            ast.nodes[child].probe_id = node.probe_id;
            return child;
          }
      ...
      if (node.child_first >= 0 && ast.nodes[node.child_first].isZeroWidth()) {
        ...
        if (child.kind == NodeKind::Group) {
          ...
          if (gcIdx >= 0) {
            ...
            if (gc.kind == NodeKind::Repeat &&
                gc.repeat_mode == node.repeat_mode) {
              ...
              if (charIdx >= 0) {
                ...
                if (ck == NodeKind::Literal || ck == NodeKind::CharClass ||
                  ...
                  if (!child.capturing &&
                    ...
                    flat.repeat_mode = node.repeat_mode;
                    flat.child_first = charIdx;
                    flat.child_last = charIdx;
          ...
          if (child.repeat_mode == node.repeat_mode) {
...
constexpr int simplifyEmptyAlternation(auto& ast, int nodeIdx) noexcept {
  ...
  switch (node.kind) {
    ...
    case NodeKind::Alternation: {
      ...
      while (child >= 0) {
        ...
        if (simplified != child) {
          ast.nodes[simplified].next_sibling = nextSib;
          ast.nodes[simplified].prev_sibling =
              prevChild >= 0 ? prevChild : kNoNode;
          if (nextSib >= 0) {
            ast.nodes[nextSib].prev_sibling = simplified;
          }
          if (prevChild >= 0) {
            ast.nodes[prevChild].next_sibling = simplified;
          } else {
            node.child_first = simplified;
          }
          child = simplified;
        }
        prevChild = child;
    ...
    case NodeKind::Lookbehind:
    case NodeKind::NegLookbehind: {
      int inner = simplifyEmptyAlternation(ast, node.child_first);
      if (inner != node.child_first) {
        node.child_first = inner;
      }
      break;
  ...
  if (node.kind == NodeKind::Alternation) {
...

    {
      ...
      while (b >= 0) {
        ...
        if (ast.nodes[b].kind == NodeKind::Empty) {
          ...
          if (newLast >= 0) {
            ast.nodes[newLast].next_sibling = b;
          } else {
            newFirst = b;
          }
          newLast = b;
      ...
      if (newLast >= 0) {
        ast.nodes[newLast].next_sibling = -1;
      }
    }
    ...
    repeat.child_first = innerNode;
    repeat.child_last = innerNode;
  ...
  switch (node.kind) {
    case NodeKind::Literal: {
      if (!node.literal.empty()) {
        FirstCharFilter f;
        f.accepts_all = false;
        // Include all characters in the literal, not just the first.
        // Suffix stripping checks each suffix char against this filter;
        // a multi-char literal like "ab" produces both 'a' and 'b'.
    ...
    case NodeKind::Group:
      return extractFollowFilter(ast, node.child_first);
    ...
    case NodeKind::Sequence: {
      ...
      while (child >= 0) {
        ...
        auto cf = extractFollowFilter(ast, child);
        if (cf.accepts_all) {
          return cf;
        }
...
constexpr void promoteToPossessive(auto& ast, int nodeIdx) noexcept {
  ...
  switch (node.kind) {
        ...
        if (ast.nodes[child].kind == NodeKind::Repeat &&
          ...
          if (isSimpleCharTest(ast, inner)) {
            ...
            while (sibling >= 0) {
              ...
              sibling = ast.nodes[sibling].next_sibling;
            }
    ...
    case NodeKind::Lookbehind:
    case NodeKind::NegLookbehind:
      promoteToPossessive(ast, node.child_first);
      break;
    case NodeKind::Literal:
    case NodeKind::CharClass:
    case NodeKind::AnyChar:
...
constexpr void applyMultilineFlag(auto& ast, int nodeIdx) noexcept {
  ...
  if (node.kind == NodeKind::Sequence || node.kind == NodeKind::Alternation) {
    ...
    applyMultilineFlag(ast, node.child_first);
  }
}
...
constexpr void applyDotAllFlag(auto& ast, int nodeIdx, bool dotAll) noexcept {
  ...
  if (node.kind == NodeKind::Sequence || node.kind == NodeKind::Alternation) {
    ...
    while (child >= 0) {
      ...
      node.kind == NodeKind::Lookbehind ||
      node.kind == NodeKind::NegLookbehind) {
    applyDotAllFlag(ast, node.child_first, dotAll);
  }
}
...
// look up the DiscriminatorEntry at that offset, build a char class from
// its union ranges, and store the char_class_index on the alternation node.
// This allows resolveCharAtOffset to resolve through nested alternations,
// enabling both the backtracking executor and NFA to use discriminator
// dispatch when outer alternation branches contain inner alternations.
// --------------------------------------------------------------------------

constexpr void materializeDiscriminatorCharClasses(
    auto& ast, int nodeIdx) noexcept {
  if (nodeIdx < 0) {
    return;
  }
  auto& node = ast.nodes[nodeIdx];
  ...
  auto* entry = findDiscriminatorEntry(
      node.valid_discriminators, node.discriminator_offset);
  if (!entry || entry->range_count <= 0) {
    return;
  }

  CharRangeSet rs;
  for (int i = 0; i < entry->range_count; ++i) {
    rs.addRange(entry->ranges[i].lo, entry->ranges[i].hi);
  }
  node.char_class_index = ast.addCharClass(rs);
}
...
// --------------------------------------------------------------------------
// reverseAst: reverse sequence child order and literal characters.
// Called BEFORE optimizeAst for reverse matching so that the optimizer
// operates on the reversed pattern structure.
// --------------------------------------------------------------------------

constexpr void reverseAstNode(auto& ast, int nodeIdx) noexcept {
  if (nodeIdx < 0) {
    return;
  }
...
constexpr void reverseAst(auto& ast, int rootIdx) noexcept {
...
constexpr int optimizeAst(
    auto& ast, int rootIdx, bool isReversed = false) noexcept {
...

  {
    ...
    computeSuffixStripLen(ast);
    stripRootLiteralSuffix(ast);

    // Flatten again — prefix/suffix stripping may expose a non-capturing
