...
// Maximum number of alphabet partition intervals. Used by DfaCoreResult
// and DfaIntervalSets to size their interval-indexed arrays.
inline constexpr int kMaxIntervals = AlphabetPartition::kMaxBoundaries + 1;
...
// Lookaround probe DFA state: a zero-width state that evaluates a probe
// and branches to pass_state (probe matches) or fail_state (probe fails).
struct DfaProbeStateInfo {
  int probe_id = -1;
  bool negated = false;
  int lookbehind_width = -1; // >= 0 for fixed-width fast path
};

// Intern a DfaTagEntry into a tag_entries array, deduplicating by value.
// Returns the index (0 = empty/no-ops). Shared by DfaProgram and DfaCoreResult.
template <typename T>
constexpr uint16_t internTagEntryImpl(
    T& container, const DfaTagEntry& e) noexcept {
  if (e.count == 0) {
    return 0;
  }
  for (int i = 1; i < container.tag_entry_count; ++i) {
    if (container.tag_entries[i] == e) {
      return static_cast<uint16_t>(i);
    }
  }
  if (container.tag_entry_count >= T::kMaxTagEntries) {
    return 0;
  }
  container.tag_entries[container.tag_entry_count] = e;
  return static_cast<uint16_t>(container.tag_entry_count++);
}
...
struct DfaProgram {
  ...
  // Lookaround probe states: zero-width DFA states that evaluate a probe
  // and branch to pass_state or fail_state. Indexed by probe state index
  // (encoded in transitions via kIsProbeStateBit).
  static constexpr int kMaxProbeStates = 8;
  DfaProbeStateInfo probe_state_info[kMaxProbeStates] = {};
  int probe_state_count = 0;
  // Per-DFA-state probe index (into probe_state_info). 0 = no probe.
  // Probes gate acceptance: a state is only accepted if its probe passes.
  // Probe indices are 1-based (0 = no probe).
  uint8_t probe_for_state[MaxDfaStates] = {};

  constexpr uint16_t internTagEntry(const DfaTagEntry& e) noexcept {
    return internTagEntryImpl(*this, e);
  }
};
...
struct DfaCoreResult {
  static constexpr int kMaxIv = kMaxIntervals;
  ...
  // Probe states created during subset construction.
  static constexpr int kMaxProbeStates = 8;
  DfaProbeStateInfo probe_state_info[kMaxProbeStates] = {};
  int probe_state_count = 0;
  // Per-DFA-state probe index: -1 = no probe gates this state's acceptance.
  int probe_state_for_dfa[MaxDfaStates] = {};
...
constexpr DfaCoreResult<MaxDfaStates> buildDfaCore(
  ...
  struct ClosureResult {
    ...
    // NFA lookaround states crossed during this closure.
    // Used to create probe DFA states.
    enum { kMaxCrossedProbes = 4 };

    struct CrossedProbe {
      int nfa_state = -1;
      int probe_id = -1;
      bool negated = false;
      int lookbehind_width = -1;
    };
    CrossedProbe crossed_probes[kMaxCrossedProbes] = {};
    int crossed_probe_count = 0;
  };
  ...
  struct Builder {
    ...
    // Dynamically allocated workspace, replacing oversized fixed arrays
    // and saving constexpr evaluation steps.
    ...
    constexpr void ensureHashesCapacity(int needed) noexcept {
      ...
      constexpr_copy_n(state_hashes_, core.state_count - 1, newH);
      delete[] state_hashes_;
      state_hashes_ = newH;
      hashes_cap_ = newCap;
    }
    ...
    constexpr ClosureResult epsilonClosure(
      ...
      while (prioCount > 0 || normCount > 0) {
        ...
        struct ClosureCtx {
          const decltype(nfa.states[0])& state;
          int s;
          int* priority_wl;
          int& prioCount;
          int* normal_wl;
          ...
          bool is_priority;
          ClosureResult& result;
          const decltype(reachMap)& reachMap;
          decltype(core)& core;
          bool atBegin;
          bool afterNewline;
          constexpr void follow(int val) {
            if (is_priority) {
              priority_wl[prioCount++] = val;
            } else {
              normal_wl[normCount++] = val;
            }
          }
        };
        ClosureCtx ctx{
        ...
        NfaStateKindSwitch<
            Case<
                ...
                [](auto& c) {
                  if (c.is_priority) {
                    if (c.state.next >= 0) {
                      c.priority_wl[c.prioCount++] = c.state.next;
                    }
                    if (c.state.alt >= 0) {
                      c.normal_wl[c.normCount++] = c.state.alt;
                    }
                  } else {
                    if (c.state.next >= 0) {
                      c.normal_wl[c.normCount++] = c.state.next;
                    }
                    if (c.state.alt >= 0) {
                      c.normal_wl[c.normCount++] = c.state.alt;
                    }
            ...
            Case<
                NfaStateKind::GroupStart,
                [](auto& c) {
                  c.result.tags.add(
                      {static_cast<int8_t>(c.state.group_id),
                       /*is_end=*/false});
                  if (c.state.next >= 0) {
                    c.follow(c.state.next);
                  }
                }>,
            Case<
                NfaStateKind::GroupEnd,
                [](auto& c) {
                  c.result.tags.add(
                      {static_cast<int8_t>(c.state.group_id), /*is_end=*/true});
                  if (c.state.next >= 0) {
                    c.follow(c.state.next);
                  }
                }>,
            Case<
                NfaStateKind::AnchorBegin,
                [](auto& c) {
                  if constexpr (Dir == Direction::Forward) {
                    if (c.atBegin && c.state.next >= 0) {
                      c.follow(c.state.next);
                    }
                  } else {
                    // Reverse: AnchorBegin is an "end edge" anchor
                    c.result.states.set(c.s);
                    if (c.state.next >= 0 &&
                        c.reachMap.matchReachable.test(c.state.next)) {
                      c.result.has_match_at_end = true;
                    }
                  }
                }>,
            Case<
                NfaStateKind::AnchorEnd,
                [](auto& c) {
                  if constexpr (Dir == Direction::Forward) {
                    c.result.states.set(c.s);
                    if (c.state.next >= 0 &&
            ...
            Case<
                NfaStateKind::AnchorStartOfString,
                [](auto& c) {
                  if constexpr (Dir == Direction::Forward) {
                    if (c.atBegin && c.state.next >= 0) {
                      c.follow(c.state.next);
            ...
            Case<
                NfaStateKind::AnchorEndOfString,
                [](auto& c) {
                  if constexpr (Dir == Direction::Forward) {
                    c.result.states.set(c.s);
                    if (c.state.next >= 0 &&
            ...
            Case<
                NfaStateKind::AnchorEndOfStringOrNewline,
                [](auto& c) {
                  if constexpr (Dir == Direction::Forward) {
                    c.result.states.set(c.s);
                    if (c.state.next >= 0 &&
                        c.reachMap.matchReachable.test(c.state.next)) {
                      c.result.has_match_at_end = true;
                      c.result.has_match_before_trailing_newline = true;
                    }
                  } else {
                    // Reverse: AnchorEndOfStringOrNewline — "start edge"
                    // anchor. Followable at the anchored start (atBegin) and
                    // also when starting at a trailing newline (afterNewline),
                    // since \Z matches both at end-of-string and before
                    // trailing newline.
                    if ((c.atBegin || c.afterNewline) && c.state.next >= 0) {
                      c.follow(c.state.next);
                    }
                ...
                [](auto& c) {
                  if constexpr (Dir == Direction::Forward) {
                    ...
                    if (c.state.next >= 0 &&
                        c.reachMap.matchReachable.test(c.state.next)) {
                      c.result.has_match_at_end = true;
                      c.result.has_match_before_newline = true;
                    }
                  }
                }>,
            Case<
                NfaStateKind::AnchorEndLine,
                [](auto& c) {
                  if constexpr (Dir == Direction::Forward) {
                    c.result.states.set(c.s);
                    if (c.state.next >= 0 &&
            ...
            Case<
                ...
                [](auto& c) {
                  c.result.states.set(c.s);
                  c.result.has_match = true;
                  if (c.is_priority) {
                    c.result.priority_match = true;
                  }
                }>,
            Case<NfaStateKind::Literal, Fallthrough>,
            Case<NfaStateKind::AnyByte, Fallthrough>,
            Case<
                NfaStateKind::CharClass,
                [](auto& c) { c.result.states.set(c.s); }>,
            Case<NfaStateKind::LookaheadProbe, Fallthrough>,
            Case<NfaStateKind::NegLookaheadProbe, Fallthrough>,
            Case<NfaStateKind::LookbehindProbe, Fallthrough>,
            Case<NfaStateKind::NegLookbehindProbe, [](auto& c) {
              // Follow unconditionally — lookaround is a zero-width
              // assertion that gates acceptance, not transitions.
              c.core.has_lookaround_probes = true;
              if (c.result.crossed_probe_count <
                  std::remove_cvref_t<decltype(c.result)>::kMaxCrossedProbes) {
                c.result.crossed_probes[c.result.crossed_probe_count++] = {
                    c.s,
                    c.state.probe_id,
                    c.state.kind == NfaStateKind::NegLookaheadProbe ||
                        c.state.kind == NfaStateKind::NegLookbehindProbe,
                    c.state.lookbehind_width};
              }
              if (c.state.next >= 0) {
                c.follow(c.state.next);
              }
            }>>::dispatch(state.kind, ctx);
      }
    ...
    constexpr void applyClosureToState(
      ...
      // Check if this state's closure contains a probe-gated Match state
      // OR if it directly crossed a lookaround probe.
      if (core.probe_state_for_dfa[dfaState] == 0) {
        // First: check crossed_probes from the epsilon closure.
        if (closure.crossed_probe_count > 0) {
          const auto& cp = closure.crossed_probes[0];
          if (core.probe_state_count <
              DfaCoreResult<MaxDfaStates>::kMaxProbeStates) {
            int probeIdx = core.probe_state_count++;
            core.probe_state_info[probeIdx] = {
                cp.probe_id, cp.negated, cp.lookbehind_width};
            core.probe_state_for_dfa[dfaState] = probeIdx + 1;
...
buildDfaFinalize(
  ...
  constexpr_copy_n(core.tag_entries, core.tag_entry_count, prog.tag_entries);

  prog.has_lookaround_probes = core.has_lookaround_probes;
  prog.probe_state_count = core.probe_state_count;
  for (int i = 0; i < core.probe_state_count; ++i) {
    prog.probe_state_info[i] = core.probe_state_info[i];
  }
  for (int s = 0; s < core.state_count; ++s) {
    prog.probe_for_state[s] = core.probe_state_for_dfa[s];
  }
...
// ---------------------------------------------------------------------------
// ProbeDfaEntry / ProbeDfaSet: per-probe DFA built from a ProbeStore entry.
// Used by DfaRunner::evaluateProbeForAcceptance for lookaround evaluation.
// ---------------------------------------------------------------------------
...
template <int MaxProbeDfas>
struct ProbeDfaSet {
  ProbeDfaEntry entries[MaxProbeDfas > 0 ? MaxProbeDfas : 1] = {};
  int count = 0;
};

// Build probe sub-DFAs for all probes in a ProbeStore. Each probe gets a
// standalone NFA built from its AST subtree, then a DFA from that NFA.
template <int MaxProbeDfas, typename ProbeStoreType>
constexpr ProbeDfaSet<MaxProbeDfas> buildProbeDfas(
    const ProbeStoreType& probeStore, bool needsProbes, bool hasNfa) noexcept {
  ProbeDfaSet<MaxProbeDfas> result;
  if (!needsProbes || !hasNfa) {
    return result;
  }
  for (int pid = 0; pid < probeStore.probe_count && pid < MaxProbeDfas; ++pid) {
    if (!probeStore.hasProbe(pid)) {
      continue;
    }
    ...
    NfaBuilder<kProbeNfaMax, Direction::Forward, ProbeStoreType, const void*>
        builder{pnfa, probeStore, nullptr};
    auto frag = builder.build(probeStore.probes[pid].root);
    NfaState matchState;
    ...
    for (int si = 0; si < pnfa.state_count; ++si) {
      ...
      if (s.kind == NfaStateKind::Literal) {
        ...
        const auto& cc = probeStore.char_classes[s.char_class_index];
        for (int r = 0; r < cc.range_count; ++r) {
          part.addBoundary(probeStore.ranges[cc.range_offset + r].lo);
          auto hi = probeStore.ranges[cc.range_offset + r].hi;
          if (hi < 255) {
    ...
    if (part.intervalCount() <= 64) {
      ...
      for (int si = 0; si < pnfa.state_count; ++si) {
        ...
        if (s.kind == NfaStateKind::Literal) {
          ...
          const auto& cc = probeStore.char_classes[s.char_class_index];
          for (int r = 0; r < cc.range_count; ++r) {
            auto lo = probeStore.ranges[cc.range_offset + r].lo;
            auto hi = probeStore.ranges[cc.range_offset + r].hi;
    ...
    // Unroll CountedRepeat states before DFA conversion.
    constexpr int kUnrolledMax = kProbeNfaMax + 16 * 36 + 16;
    auto unrolled = unrollCountedRepeats<kUnrolledMax>(pnfa);

    // Build DFA from probe NFA
    auto ivSets = buildDfaIntervalSets(unrolled);
    auto reachMap = buildDfaReachMap(unrolled);
    auto core =
        buildDfaCore<ProbeDfaEntry::kMaxStates, false, Direction::Forward>(
            unrolled, probeStore, ivSets, reachMap);
    if (!core.valid) {
      continue;
    }
    auto dfaProg =
        buildDfaFinalize<ProbeDfaEntry::kMaxStates, ProbeDfaEntry::kMaxClasses>(
            core, unrolled);
    if (!dfaProg.valid) {
      continue;
    }
    ...
    entry.valid = true;
    entry.dir = probeStore.probes[pid].dir;
