...
enum class NfaStateKind : uint8_t {
  ...
  GroupStart,
  GroupEnd,
  LookaheadProbe, // Follow next if forward probe matches at pos
  NegLookaheadProbe, // Follow next if forward probe doesn't match at pos
  LookbehindProbe, // Follow next if reverse probe matches ending at pos
  NegLookbehindProbe, // Follow next if reverse probe doesn't match ending at
                      // pos
};

template <typename... Cases>
using NfaStateKindSwitch = ConstexprSwitchDispatcher<
    static_cast<int>(NfaStateKind::NegLookbehindProbe),
    Cases...>;
...
struct NfaState {
  ...
  // Index into the program's possessive_probe array. When >= 0 and
  // matching in reverse, the possessive exit runs a forward NFA probe.
  // If the probe matches, the exit thread is killed.
  int possessive_probe_idx = -1;
  // Probe ID for lookaround states — references ProbeStore entry.
  // Separate from possessive_probe_idx to avoid conflation.
  int probe_id = -1;
  // Fixed-width for lookbehind fast path. -1 = variable-width (use probe).
...
struct NfaProgram {
  ...
  // Embedded sub-NFA programs for possessive forward probes.
  // Each probe is a small forward NFA built from a possessive repeat's
  // inner pattern. Stored as ranges within the same states array but
  // with separate start states.
  static constexpr int kMaxProbes = 8;
  int probe_start[kMaxProbes] = {};
  int probe_count = 0;

  // Embedded sub-NFA start states for lookaround probes, indexed by
  // probe_id. -1 means no embedded NFA for that probe_id.
  static constexpr int kMaxLookaroundProbes = 16;
  int lookaround_probe_start[kMaxLookaroundProbes] = {};
  int lookaround_probe_count = 0;
  ...
  constexpr int addProbe(int startState) noexcept {
    int idx = probe_count++;
    if (idx < kMaxProbes) {
      probe_start[idx] = startState;
    }
    return idx;
  }
};
...
template <
    ...
    Direction Dir,
    typename AstType,
    typename ForwardAstPtrType>
struct NfaBuilder {
  NfaProgram<MaxStates>& prog;
  const AstType& ast;
  ForwardAstPtrType forwardAst_;
  ...
  constexpr int packDiscriminatorChar(int astChildIdx, int offset) noexcept {
    ...
    } else if (n.char_class_index >= 0) {
      return 256 + n.char_class_index;
    }
    return -1;
  }

  // Remap char_class_index values for states added between startIdx and
  // prog.state_count. Probe sub-NFA states reference forwardAst_'s
  // char_classes[], but the partition/mask computation uses ast's. This
  // remaps each probe state's index to the matching char class in ast.
  constexpr void remapProbeCharClasses(int startIdx) noexcept {
    if constexpr (std::is_void_v<std::remove_pointer_t<ForwardAstPtrType>>) {
      return;
    } else {
      if (!forwardAst_) {
        return;
      }
      for (int i = startIdx; i < prog.state_count; ++i) {
        auto& s = prog.states[i];
        if (s.kind != NfaStateKind::CharClass || s.char_class_index < 0) {
          continue;
        }
        int probeIdx = s.char_class_index;
        if (probeIdx >= forwardAst_->char_class_count) {
          continue;
        }
        const auto& probeCC = forwardAst_->char_classes[probeIdx];
        // Find matching char class in the main AST by range comparison.
        for (int j = 0; j < ast.char_class_count; ++j) {
          const auto& mainCC = ast.char_classes[j];
          if (mainCC.range_count != probeCC.range_count) {
            continue;
          }
          bool match = true;
          for (int r = 0; r < mainCC.range_count; ++r) {
            if (ast.ranges[mainCC.range_offset + r].lo !=
                    forwardAst_->ranges[probeCC.range_offset + r].lo ||
                ast.ranges[mainCC.range_offset + r].hi !=
                    forwardAst_->ranges[probeCC.range_offset + r].hi) {
              match = false;
              break;
            }
          }
          if (match) {
            s.char_class_index = j;
            break;
          }
        }
      }
  ...
  // Build a forward probe sub-NFA for a possessive repeat's inner.
  // Looks up the probe's root node directly from the ProbeStore.
  constexpr int buildPossessiveProbe(int innerIdx, int probeId) noexcept {
    if constexpr (std::is_void_v<std::remove_pointer_t<ForwardAstPtrType>>) {
      return -1;
    } else {
      if (innerIdx < 0 || probeId < 0) {
        return -1;
      }
      if (!forwardAst_ || !forwardAst_->hasProbe(probeId)) {
        return -1;
      }
      int fwdInnerIdx = forwardAst_->probes[probeId].root;
      if (fwdInnerIdx < 0) {
        return -1;
      }
      using FwdAstType = std::remove_cvref_t<decltype(*forwardAst_)>;
      NfaBuilder<MaxStates, Direction::Forward, FwdAstType, ForwardAstPtrType>
          fwdBuilder{prog, *forwardAst_, forwardAst_};
      int statesBefore = prog.state_count;
      NfaFragment inner = fwdBuilder.build(fwdInnerIdx);
      remapProbeCharClasses(statesBefore);
      NfaState matchState;
      matchState.kind = NfaStateKind::Match;
      int matchIdx = prog.addState(matchState);
      patch(inner.end, matchIdx);
      return prog.addProbe(inner.start);
    }
  }

  // Build a sub-NFA for a lookaround probe and store its start
  // state in the lookaround_probe_start array, indexed by probe_id.
  // Lookahead probes build forward sub-NFAs; variable-width lookbehind
  // probes build reverse sub-NFAs from the reverse-optimized AST.
  constexpr void buildLookaroundProbe(int probeId) noexcept {
    if constexpr (std::is_void_v<std::remove_pointer_t<ForwardAstPtrType>>) {
      return;
    } else {
      if (probeId < 0 || !forwardAst_ || !forwardAst_->hasProbe(probeId)) {
        return;
      }
      int probeRoot = forwardAst_->probes[probeId].root;
      if (probeRoot < 0) {
        return;
      }

      auto registerProbe = [&](NfaFragment inner) {
        NfaState matchState;
        matchState.kind = NfaStateKind::Match;
        int matchIdx = prog.addState(matchState);
        patch(inner.end, matchIdx);
        if (probeId < NfaProgram<MaxStates>::kMaxLookaroundProbes) {
          prog.lookaround_probe_start[probeId] = inner.start;
          if (probeId >= prog.lookaround_probe_count) {
            prog.lookaround_probe_count = probeId + 1;
          }
        }
      };

      using ProbeAstType = std::remove_cvref_t<decltype(*forwardAst_)>;
      if (forwardAst_->probes[probeId].dir == Direction::Reverse) {
        NfaBuilder<
            MaxStates,
            Direction::Reverse,
            ProbeAstType,
            ForwardAstPtrType>
            builder{prog, *forwardAst_, forwardAst_};
        int statesBefore = prog.state_count;
        auto frag = builder.build(probeRoot);
        remapProbeCharClasses(statesBefore);
        registerProbe(frag);
      } else {
        // Forward probe: build a forward sub-NFA
        NfaBuilder<
            MaxStates,
            Direction::Forward,
            ProbeAstType,
  ...
  constexpr NfaFragment build(int nodeIdx) noexcept {
    ...
    switch (node.kind) {
      ...
      case NodeKind::Literal: {
        ...
        int firstState = -1;
        int prevState = -1;
        int litLen = static_cast<int>(lit.size());
      ...
      case NodeKind::Group: {
        NfaFragment inner = build(node.child_first);
        if (node.capturing) {
          NfaState gs;
      ...
      case NodeKind::Alternation: {
        ...
        NfaFragment first = build(child);
        int nextChild = ast.nodes[child].next_sibling;
        ...
        int moreChild = ast.nodes[nextChild].next_sibling;
        while (moreChild >= 0) {
          ...
          moreChild = ast.nodes[moreChild].next_sibling;
        }
      ...
      case NodeKind::Repeat: {
        ...
        bool greedy = node.repeat_mode != RepeatMode::Lazy;
        ...
        if (minR == 0 && maxR == 1) {
          ...
          NfaState split;
          split.kind = NfaStateKind::Split;
          split.repeat_mode = node.repeat_mode;
          if (node.isPossessive()) {
            prog.has_possessive = true;
            if constexpr (Dir == Direction::Reverse) {
              if (node.isPossessiveProbed()) {
                split.possessive_probe_idx =
                    buildPossessiveProbe(innerIdx, node.probe_id);
              }
            }
          }
          if (greedy) {
        ...
        if (minR == 1 && maxR < 0) {
          ...
          // For possessive +, add an entry Split before inner.start so
          // that possessive_entry_pos is captured at the correct position
          // (before the first body iteration, not after it).
          if (node.isPossessive()) {
            NfaState entry;
            entry.kind = NfaStateKind::Split;
            entry.next = inner.start;
            entry.alt = -1;
            entry.repeat_mode = node.repeat_mode;
            if constexpr (Dir == Direction::Reverse) {
              entry.possessive_probe_idx = split.possessive_probe_idx;
            }
            int entryIdx = prog.addState(entry);
            return {entryIdx, outIdx};
          }
        ...
        cr.min_repeat = minR;
        cr.max_repeat = maxR;
        cr.repeat_mode = node.repeat_mode;
        if (node.isPossessive()) {
          prog.has_possessive = true;
          if constexpr (Dir == Direction::Reverse) {
            if (node.isPossessiveProbed()) {
              cr.possessive_probe_idx =
                  buildPossessiveProbe(innerIdx, node.probe_id);
            }
          }
        }
      ...
      case NodeKind::Lookahead: {
        NfaState s;
        s.kind = NfaStateKind::LookaheadProbe;
        s.probe_id = node.probe_id;
        s.next = -1;
        int idx = prog.addState(s);
        prog.has_lookaround_probes = true;
        buildLookaroundProbe(node.probe_id);
        return {idx, idx};
      }
      case NodeKind::NegLookahead: {
        NfaState s;
        s.kind = NfaStateKind::NegLookaheadProbe;
        s.probe_id = node.probe_id;
        s.next = -1;
        int idx = prog.addState(s);
        prog.has_lookaround_probes = true;
      ...
      case NodeKind::Lookbehind: {
        NfaState s;
        s.kind = NfaStateKind::LookbehindProbe;
        s.probe_id = node.probe_id;
        s.lookbehind_width = node.min_repeat; // -1 for variable
        s.next = -1;
        int idx = prog.addState(s);
        prog.has_lookaround_probes = true;
        buildLookaroundProbe(node.probe_id);
      ...
      case NodeKind::NegLookbehind: {
        NfaState s;
        s.kind = NfaStateKind::NegLookbehindProbe;
        s.probe_id = node.probe_id;
        s.lookbehind_width = node.min_repeat; // -1 for variable
        s.next = -1;
        int idx = prog.addState(s);
...
template <int MaxStates, Direction Dir = Direction::Forward>
constexpr NfaProgram<MaxStates> buildNfa(const auto& ast) noexcept {
  // Use void* as ForwardAstPtrType so that buildPossessiveProbe's
  // if constexpr (std::is_void_v<...>) check short-circuits.
  return buildNfa<MaxStates, Dir>(ast, static_cast<const void*>(nullptr));
}

template <int MaxStates, Direction Dir = Direction::Forward>
constexpr NfaProgram<MaxStates> buildNfa(
    const auto& ast, const auto* forwardAst) noexcept {
  NfaProgram<MaxStates> prog;

  using AstType = std::remove_cvref_t<decltype(ast)>;
  using ForwardAstPtrType = decltype(forwardAst);
  using Builder = NfaBuilder<MaxStates, Dir, AstType, ForwardAstPtrType>;

  Builder builder{prog, ast, forwardAst};
...
constexpr NfaProgram<MaxOutputStates> unrollCountedRepeats(
  ...
  constexpr_copy_n(input.states, input.state_count, out.states);
  ...
  out.has_lookaround_probes = input.has_lookaround_probes;
  out.lookaround_probe_count = input.lookaround_probe_count;
  for (int i = 0; i < input.lookaround_probe_count; ++i) {
    out.lookaround_probe_start[i] = input.lookaround_probe_start[i];
  }
  ...
  struct Unroller {
    ...
    constexpr bool unroll() noexcept {
      ...
      bool greedy = cr.repeat_mode != RepeatMode::Lazy;
  ...
  for (int ci = 0; ci < crCount; ++ci) {
    ...
    Unroller u{
        out, crs[ci], crs[ci].index, out.states[crs[ci].index].repeat_mode};
    if (!u.unroll()) {
      crs[ci].shouldUnroll = false;
    }
  }
