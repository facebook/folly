...
constexpr bool hasAnchorBegin(const auto& ast, int nodeIdx) noexcept {
  ...
  if (node.kind == NodeKind::Anchor && node.anchor == AnchorKind::Begin) {
    return true;
  }
...
struct CharTestTrait {
  static constexpr bool available = [] {
    if constexpr (Idx < 0) {
      ...
      return kind == NodeKind::Literal || kind == NodeKind::AnyChar ||
          kind == NodeKind::AnyByte || kind == NodeKind::CharClass;
    }
  }();
  ...
  static bool testChar(char c) noexcept {
    if constexpr (Idx < 0) {
      ...
      if constexpr (n.kind == NodeKind::Literal) {
        return c == n.ch;
      } else if constexpr (n.kind == NodeKind::AnyChar) {
        return true;
        ...
        if constexpr (cc.range_count == 1) {
          // Single range: 2 comparisons, no memory load
          constexpr auto r = Ast.ranges[cc.range_offset];
          auto uc = static_cast<unsigned char>(c);
          bool hit = (uc >= r.lo && uc <= r.hi);
          if constexpr (cc.negated) {
            return !hit;
          } else {
            return hit;
          }
          ...
          auto uc = static_cast<unsigned char>(c);
          bool hit =
              (uc >= r0.lo && uc <= r0.hi) || (uc >= r1.lo && uc <= r1.hi);
          if constexpr (cc.negated) {
            return !hit;
          } else {
            return hit;
          ...
          auto uc = static_cast<unsigned char>(c);
          bool hit = (uc >= r0.lo && uc <= r0.hi) ||
              (uc >= r1.lo && uc <= r1.hi) || (uc >= r2.lo && uc <= r2.hi);
          if constexpr (cc.negated) {
            return !hit;
          } else {
            return hit;
...
constexpr FirstCharFilter extractFirstCharFilter(
  ...
  switch (node.kind) {
    case NodeKind::Literal: {
      FirstCharFilter f;
      f.accepts_all = false;
      f.addChar(static_cast<unsigned char>(node.ch));
      return f;
    }
    case NodeKind::CharClass: {
      const auto& cc = ast.char_classes[node.char_class_index];
      if (cc.negated || cc.range_count > FirstCharFilter::kMaxRanges) {
        return {.accepts_all = true};
      }
      FirstCharFilter f;
      f.accepts_all = false;
...
constexpr void extractLiteralPrefixImpl(
    const auto& ast, int nodeIdx, LiteralSpan<>& result) noexcept {
  if (nodeIdx < 0) {
    return;
  }
  ...
  switch (node.kind) {
    ...
    case NodeKind::Sequence: {
      ...
      while (child >= 0) {
        ...
        extractLiteralPrefixImpl(ast, child, result);
        if (result.length == before) {
          return;
        }
        // Only continue through Literal children — non-Literal
        // children (Repeat, Group, etc.) are variable-width, so the
        // next child's chars are not at a fixed offset from the start.
        if (ast.nodes[child].kind != NodeKind::Literal) {
          return;
        }
        child = ast.nodes[child].child_end;
      }
    ...
    case NodeKind::Group:
      extractLiteralPrefixImpl(ast, node.child_begin, result);
      return;
    case NodeKind::Repeat:
      if (node.min_repeat > 0) {
        extractLiteralPrefixImpl(ast, node.child_begin, result);
...
constexpr LiteralSpan<> extractLiteralPrefix(
    const auto& ast, int nodeIdx) noexcept {
  LiteralSpan<> result;
  extractLiteralPrefixImpl(ast, nodeIdx, result);
  return result;
}

constexpr void extractLiteralSuffixImpl(
    const auto& ast, int nodeIdx, LiteralSpan<>& result) noexcept {
  if (nodeIdx < 0) {
    return;
  }
  ...
  switch (node.kind) {
    ...
    case NodeKind::Sequence: {
      ...
      for (int i = count - 1; i >= 0; --i) {
        int before = result.length;
        extractLiteralSuffixImpl(ast, children[i], result);
        if (result.length == before) {
          return;
        }
        // Only continue through Literal children — non-Literal
        // children are variable-width.
    ...
    case NodeKind::Group:
      extractLiteralSuffixImpl(ast, node.child_begin, result);
      return;
    case NodeKind::Repeat:
      if (node.min_repeat > 0) {
        extractLiteralSuffixImpl(ast, node.child_begin, result);
...
constexpr LiteralSpan<> extractLiteralSuffix(
    const auto& ast, int nodeIdx) noexcept {
  LiteralSpan<> span;
  extractLiteralSuffixImpl(ast, nodeIdx, span);
  // Reverse since we collected right-to-left
  for (int i = 0; i < span.length / 2; ++i) {
    char tmp = span.chars[i];
    span.chars[i] = span.chars[span.length - 1 - i];
    span.chars[span.length - 1 - i] = tmp;
  }
  return span;
}
...
struct BacktrackExecutor {
  ...
  static bool match(
    ...
    if constexpr (node.kind == NodeKind::Empty) {
      ...
      if (pos < input.size() && input[pos] == node.ch) {
        return cont(pos + 1);
      }
      return false;
    } else if constexpr (node.kind == NodeKind::AnyChar) {
      if (pos < input.size()) {
        return cont(pos + 1);
      }
      return false;
      ...
      if constexpr (TrackCaptures && node.capturing) {
        ...
        return BacktrackExecutor<
            ...
            node.child_begin,
            TrackCaptures,
            NumGroups>::match(input, pos, state, budget, cont);
      }
      ...
      bool innerMatched = BacktrackExecutor<
          ...
          node.child_begin,
          TrackCaptures,
          NumGroups>::match(input, pos, state, budget, [](std::size_t) {
        return true;
      });
      state = savedState;
      if (innerMatched) {
  ...
  static bool matchSequence(
    ...
    if constexpr (ChildIdx < 0) {
      ...
      return BacktrackExecutor<Ast, F, ChildIdx, TrackCaptures, NumGroups>::
  ...
  static bool matchAlternation(
    ...
    if constexpr (AltIdx < 0) {
      ...
      if (BacktrackExecutor<Ast, F, AltIdx, TrackCaptures, NumGroups>::match(
              input, pos, state, budget, cont)) {
        return true;
      }
  ...
  static bool matchRepeatGreedy(
    ...
    if (BacktrackExecutor<Ast, F, InnerIdx, TrackCaptures, NumGroups>::match(
            input, pos, state, budget, [&](std::size_t nextPos) {
              matchedPos = nextPos;
              return true;
            })) {
      if (matchedPos != pos) {
        if (matchRepeatGreedy<InnerIdx, MinR, MaxR>(
  ...
  static bool matchRepeatLazy(
    ...
    return BacktrackExecutor<Ast, F, InnerIdx, TrackCaptures, NumGroups>::match(
        input, pos, state, budget, [&](std::size_t nextPos) {
          if (nextPos == pos) {
            return false;
          }
...
struct BacktrackRunner {
  ...
  // MatchOutcome with the appropriate state size.
  ...
  static auto search(std::string_view input) noexcept {
    ...
    auto tryMatch = [&](std::size_t start) {
      ...
      bool matched =
          BacktrackExecutor<Ast, F, Ast.root, kBoolOnlyCaptures, kStateGroups>::
              match(input, start, state, budgetCopy, [](std::size_t) {
                return true;
              });
      ...
      // Sentinel: not matched, not exhausted — caller should continue
      if constexpr (BoolOnly) {
        return SearchBoolResult{false, false};
      } else {
        return decltype(makeMatch()){};
      }
    };
  ...
  static MatchOutcome<NumGroups> searchCapturing(
      std::string_view input) noexcept {
    constexpr bool anchored = hasAnchorBegin(Ast, Ast.root);
    constexpr auto kFilter = extractFirstCharFilter(Ast, Ast.root);
    constexpr auto kReqLit = extractRequiredLiteral(Ast, Ast.root);
    ...
    if constexpr (useMemchr) {
      ...
      while (scan < end) {
        ...
        for (std::size_t start = tryStart; start <= litPos && start <= maxStart;
          ...
          bool matched =
              BacktrackExecutor<Ast, F, Ast.root, TrackCaptures, NumGroups>::
                  match(
                      input, start, state, budgetCopy, [&](std::size_t endPos) {
                        matchEnd = endPos;
                        return true;
                      });
          ...
          if (matched) {
            ...
            outcome.status = MatchStatus::Matched;
            if constexpr (TrackCaptures) {
              state.groups[0] = {start, matchEnd - start};
            }
            outcome.state = state;
            return outcome;
          }
