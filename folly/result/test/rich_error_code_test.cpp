...
constexpr bool testPlain() {
  ...
  check(get_rich_error_code<A1>(ec) == A1::ONE_A1);
  check(get_rich_error_code<B1>(ec) == B1{ImplB1::TWO_B1});
  check(get_rich_error_code<B2>(ec) == B2::ONE_B2);
  check(get_rich_error_code<C1>(ec) == C1::TWO_C1);
  return true;
}
...
constexpr bool testPolymorphic() {
  ...
  check(get_rich_error_code<A1>(ea) == A1::ZERO_A1);
  check(get_rich_error_code<B1>(ea) == std::nullopt);
  check(get_rich_error_code<B2>(ea) == std::nullopt);
  check(get_rich_error_code<C1>(ea) == std::nullopt);

  constexpr auto eb_ptr =
      immortal_rich_error<ErrorB, A1::ONE_A1, B1{ImplB1::ONE_B1}, B2::TWO_B2>.ptr();
  auto& eb = *get_rich_error(eb_ptr);
  check(get_rich_error_code<A1>(eb) == A1::ONE_A1);
  check(get_rich_error_code<B1>(eb) == B1{ImplB1::ONE_B1});
  check(get_rich_error_code<B2>(eb) == B2::TWO_B2);
  check(get_rich_error_code<C1>(eb) == std::nullopt);
  ...
  constexpr auto ec_ptr = immortal_rich_error<
      ...
      C1::ONE_C1>.ptr();
  auto& ec = *get_rich_error(ec_ptr);
  check(get_rich_error_code<A1>(ec) == A1::TWO_A1);
  check(get_rich_error_code<B1>(ec) == B1{ImplB1::ZERO_B1});
  check(get_rich_error_code<B2>(ec) == B2::ZERO_B2);
  check(get_rich_error_code<C1>(ec) == C1::ONE_C1);
...
TEST(RichErrorCodeTest, Fmt) {
  ...
  EXPECT_EQ(
      ...
      fmt::format("folly::test::C1=101, B1=10, B2=-20, A1=2"));
}
...
struct rich_error_code<test::BadHasReservedUuid> {
...
namespace folly {
template <> // NOLINT(facebook-hte-MisplacedTemplateSpecialization)
struct rich_error_code<test::BadHasDuplicateUuid> {
  static constexpr uint64_t uuid =
      rich_error_code<test::A1>::uuid; // BUG: duplicate
};
} // namespace folly
