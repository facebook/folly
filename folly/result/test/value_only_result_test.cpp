...
void checkAwaitResumeTypeForRefResult() {
  ...
  static_assert( // `co_await`
      std::is_same_v<
          T&&,
          decltype(fakeCoAwait(
              std::move(FOLLY_DECLVAL(value_only_result<T&&>&&))))>);
  ...
  static_assert( // `co_await std::ref(r)`
      std::is_same_v<
          T&,
          decltype(fakeCoAwait(
              std::ref(FOLLY_DECLVAL(value_only_result<T&>&))))>);
  static_assert( // `co_await std::cref(r)`
      std::is_same_v<
          T&,
          decltype(fakeCoAwait(
              std::cref(FOLLY_DECLVAL(value_only_result<T&>&))))>);
  static_assert( // `co_await std::move(r)`
      std::is_same_v<
          T&,
          decltype(fakeCoAwait(FOLLY_DECLVAL(value_only_result<T&>&&)))>);
...
RESULT_CO_TEST(ValueOnlyResult, copySmallTrivialUnderlying) {
  ...
  EXPECT_EQ(s, co_await value_only_result<Smol>{s});
}
