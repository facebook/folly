...
int ehFrameHdrCallback(struct dl_phdr_info* info, size_t, void* data) {
  ...
  for (int i = 0; i < info->dlpi_phnum; i++) {
    if (info->dlpi_phdr[i].p_type == PT_GNU_EH_FRAME) {
      // dlpi_addr is the module's integer load address (0 for a non-PIE main
      // executable); combine it with p_vaddr in integer space so we don't rely
      // on nullptr + offset pointer arithmetic, then cast once at the end.
      // NOLINTNEXTLINE(performance-no-int-to-ptr)
      const auto* hdr = reinterpret_cast<const uint8_t*>(
          static_cast<std::uintptr_t>(info->dlpi_addr) +
          static_cast<std::uintptr_t>(info->dlpi_phdr[i].p_vaddr));
      result->found = true;
