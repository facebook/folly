...
class IoUringProvidedBufferRingTestHelper {
 ...
 private:
  uint32_t areaIndexOf(const IoUringProvidedBufferRing::BufferArea* area) {
    for (uint32_t a = 0; a < ring.areaCount_; a++) {
...
TEST_F(IoUringProvidedBufferRingTest, BufferCountCheck) {
  ...
  auto makeOptions = [&bgid](uint32_t bufferCount) {
    return IoUringProvidedBufferRing::Options{
        .gid = bgid++,
...
static std::unique_ptr<folly::IOBuf> consumeOne(
    folly::IoUringProvidedBufferRing& ring,
    folly::IoUringProvidedBufferRingTestHelper& helper,
    size_t len) {
  uint16_t bid = helper.headBid();
  return ring.getIoBuf(bid, len, false);
}

TEST_F(IoUringProvidedBufferRingTest, GrowsOnExhaustion) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, BundleFollowsRefillOrderAcrossReuse) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, ReuseStabilizesWithoutUnboundedGrowth) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, GrowthIsCappedWhenAllBuffersHeld) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, ReuseOnlyWhenFullyDrained) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, activeAreaMatchesRefillOrder) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, ContiguityWithinFullRingLoad) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, BundleAcrossAreaBoundary) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, ShrinkAfterGrowthDownToFloor) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, NoShrinkWhenLessThanHalfFreeSlots) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, ShrinkWhenMoreThanHalfFreeSlots) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, ShrinkNeverRemovesActiveOrRefillArea) {
  io_uring ring{};
  io_uring_queue_init(512, &ring, 0);
  IoUringProvidedBufferRing::Options options = {
      .gid = 1,
...
TEST_F(IoUringProvidedBufferRingTest, ringRefillThresholdBatchesRefills) {
