...
TEST(FixedStringCtorTest, FromUDL) {
  ...
  static_assert(std::is_same<decltype(x), const folly::FixedString<5>>::value);
  static_assert(x[0] == 'h');
  static_assert(x[1] == 'e');
  static_assert(x[2] == 'l');
  static_assert(x[3] == 'l');
  ...
  static_assert(std::is_same<decltype(y), const folly::FixedString<8>>::value);
  static_assert(y.size() == 7u);
  static_assert(y == "goodbye");

  constexpr auto z = "now is the time for all good llamas"_fs64;
  static_assert(std::is_same<decltype(z), const folly::FixedString<64>>::value);
