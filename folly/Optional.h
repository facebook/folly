...
class Optional {
 ...
 private:
  ...
  struct StorageNonTriviallyDestructible {
   ...
   public:
    ...
    void clear() {
      if (hasValue_) {
        hasValue_ = false;
        folly::launder(reinterpret_cast<Value*>(value_))->~Value();
      }
    }
  };
  ...
  struct Storage : std::conditional<
    ...
    Value* value_pointer() {
      if (this->hasValue_) {
        return folly::launder(reinterpret_cast<Value*>(this->value_));
      }
      return nullptr;
    }
    ...
    Value const* value_pointer() const {
      if (this->hasValue_) {
        return folly::launder(reinterpret_cast<Value const*>(this->value_));
      }
      return nullptr;
    }
