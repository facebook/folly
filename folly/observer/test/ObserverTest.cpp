...
// Regression test for a shutdown-order race: SingletonVault::destroyInstances()
// must not tear down ObserverManager's internal queue processors out from
// under a caller that is concurrently, and still genuinely, inside a
// waitForAllUpdates() call (which dereferences them). Before the fix this was
// unsynchronized: in production this manifested as ObserverManager's refresh
// threads still touching config data that a shutdown teardown was
// concurrently freeing elsewhere (a real fbagentc/dynamo/smcc crash family);
// here it is exercised directly and deterministically via TSAN + a real
// (non-trivial) race window instead of relying on a fleet crash to notice.
TEST(Observer, ShutdownWhileWaitForAllUpdatesInFlight) {
  folly::observer::SimpleObservable<int> observable{1};

  // Gate the update recompute on a baton instead of a timed sleep, so it is
  // deterministically still in-flight (not already finished, and not merely
  // "probably still running") when destroyInstances() and the concurrent
  // waitForAllUpdates() race below. Only the update recompute is gated; the
  // initial computation must run to completion so the observer has a value.
  folly::Baton<> recomputeStarted;
  folly::Baton<> allowRecomputeToFinish;
  std::atomic<bool> gateRecompute{false};

  auto slowObserver = folly::observer::makeObserver(
      [o = observable.getObserver(),
       &recomputeStarted,
       &allowRecomputeToFinish,
       &gateRecompute] {
        if (gateRecompute.exchange(false)) {
          recomputeStarted.post();
          allowRecomputeToFinish.wait();
        }
        return **o;
      });
  EXPECT_EQ(1, **slowObserver);
  ...
  // The update recompute is now genuinely in-flight and parked, so the
  // waitForAllUpdates() call below is guaranteed to block on it rather than
  // return immediately.
  recomputeStarted.wait();

  folly::Baton<> waiterAboutToWait;
  std::thread waiter([&waiterAboutToWait] {
    // Signalled immediately before the call so the main thread can order
    // shutdown after the waiter has entered waitForAllUpdates(), without a
    // timed sleep. The parked recompute above guarantees the waiter then
    // blocks inside the call until the gate is released.
    waiterAboutToWait.post();
    folly::observer_detail::ObserverManager::waitForAllUpdates();
  });
  waiterAboutToWait.wait();

  // Release the gate from a separate thread: destroyInstances() below joins
  // the queue-processor thread that is running the parked recompute, so the
  // recompute must be allowed to finish *concurrently* with shutdown (not
  // before it) -- releasing it here keeps the race live while still letting
  // shutdown make progress rather than deadlocking on the join.
  std::thread releaser([&allowRecomputeToFinish] {
    allowRecomputeToFinish.post();
  });
  ...
  // Machinery must still work after shutdown (mirrors TEST(Observer,
  // Shutdown) above).
  auto freshObserver = folly::observer::makeObserver([] { return 42; });
  EXPECT_EQ(42, **freshObserver);
}
