...
folly::coro::Task<Response> handle(Request request) {
  co_return co_await folly::coro::co_withMetadata(
      request.tracingId(), process(std::move(request)));
}
```
...
active while the wrapped task runs and is removed when that task completes,
throws, or is cancelled or destroyed.
...
The final public C++ name and exact signature of that presence-aware slot are
not specified here. Code must not depend on the older convention in which a
parallel integer array used zero for untagged frames.
...
The metadata-aware in-process async-stack reader returns one metadata slot for
each displayed stack address. Synthetic metadata frames are an internal
encoding: they do not appear as addresses and must not be symbolized. An
address-only reader returns the ordinary stack with those synthetic frames
removed.
...
until there are concrete 32-bit use cases. This does not make all of Folly fail
to build on 32-bit platforms.
...
No uniform metadata-reader contract across Windows, macOS, libc++, and
libstdc++ is established here. The coroutine wrapper itself does not require a
specific standard-library implementation, but debugger and profiler support is
