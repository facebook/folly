...
Start with `input-map.md`. Use only the staged packet. Later user decisions
override earlier discussion, review conclusions, and frozen source. Describe the
latest intended behavior, not its history.
...
`output.md`. Do not inspect parent directories, a live checkout, source control,
earlier outputs, evaluator material, or network content.
